...
def analyze_labels(df, split_name="Overall"):
    ...
    print(f"{'-'*80}")
    print(f"{'Label':<10} {'Count':<10} {'Percentage':<12} {'Positive':<10} {'Negative':<10}")
    print(f"{'-'*80}")
    ...
    for label in label_cols:
        ...
        print(f"{label:<10} {len(df):<10} {percentage:>6.2f}%      {count_positive:<10} {count_negative:<10}")
    ...
    for i, label1 in enumerate(label_cols):
        for j, label2 in enumerate(label_cols):
            if i <= j:
                cooccurrence.loc[label1, label2] = ((df[label1] == 1.0) & (df[label2] == 1.0)).sum()
                if i != j:
    ...
    for rank, (combination, count) in enumerate(combination_counts.items(), 1):
        percentage = (count / len(df)) * 100
        labels_present = [label_cols[i] for i, val in enumerate(combination) if val == 1.0]
...
def main():
    ...
    # Load the data
    csv_path = Path(__file__).parent / "split.csv"
