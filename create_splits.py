...
from dataset import _get_s3_client, read_csv_from_s3, list_sets
...
def _assign_spatial_splits(labels_df: pd.DataFrame, locations_df: pd.DataFrame, train_ratio: float, val_ratio: float):
    ...
    result = result.merge(geo_df[["patch_id", "split"]], on="patch_id", how="left", suffixes=("", "_geo"))
...
def _assign_fallback_splits(labels_df: pd.DataFrame, train_ratio: float, val_ratio: float, seed: int):
    ...
    labels_df.iloc[indices[n_train:n_train + n_val], labels_df.columns.get_loc("split")] = "val"
    labels_df.iloc[indices[n_train + n_val:], labels_df.columns.get_loc("split")] = "test"
    return labels_df
...
def create_split(
    ...
    if not force:
        try:
            ...
            print(f"  ✔ split.csv already exists ({n_train}/{n_val}/{n_test}). Use --force to overwrite.")
            return existing
        except Exception:
            pass  # No existing split, proceed
    ...
    if locations_df is not None:
        split_df = _assign_spatial_splits(labels_df, locations_df, train_ratio, val_ratio)
        if split_df is not None and split_df["split"].notna().all():
            labels_df = split_df
            print(f"  Spatial split source: {set_prefix}locations.geojson")
        else:
            print("  ! Falling back to random split because locations could not be joined")
            labels_df = _assign_fallback_splits(labels_df, train_ratio, val_ratio, seed)
    else:
        print("  ! Falling back to random split because locations.geojson is unavailable")
        labels_df = _assign_fallback_splits(labels_df, train_ratio, val_ratio, seed)
...
def main():
    parser = argparse.ArgumentParser(description="Create train/val/test splits for EarthScape")
    ...
    base_prefixes = cfg["data"].get("base_prefixes", [cfg["data"].get("base_prefix", "")])
